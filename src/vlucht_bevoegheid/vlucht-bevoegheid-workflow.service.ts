  ...
  async run(baseDate = new Date()): Promise<void> {
    ...
    for (const lidId of vliegers) {
      ...
      if (!onbevoegdIds.size) {
        continue;                               // alles is in orde, geen email nodig
      }

      // Bouw callsigns lijst voor de email
      const callsigns = [...onbevoegdIds]
        .map((id) => clubVliegtuigenById.get(id)?.CALLSIGN)
        .filter(Boolean)
        .join(', ');

      // Als we geen email adres hebben, dan ICT en CIMT verwittigen
      if (!lid.EMAIL) {
        const html = buildEmailErrorHtml("Bevoegdheden, geen email", `<p>${lid.NAAM} heeft gevlogen op ${datum} zonder bevoegheid op ${callsigns}, maar heeft geen emailadres. Onderneem aktie</p>`);
        await this.googleService.sendHtmlEmail({
          to: process.env.ICT || 'ict@gezc.org',
          cc: [process.env.CIMT_EMAIL || 'cimt@gezc.org'],
          subject: 'Bevoegdheid, email ontbeekt',
          html
        });
