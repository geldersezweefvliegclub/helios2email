...
export class HerinneringDagrapportWorkflowService
{
  ...
  async run(forDate = new Date()): Promise<void> {
    ...
    // Doorloop over de diensten en stuur een herinnering e-mail naar degene die een dienst heeft, maar alleen als het een dienst is waarbij een dagrapport geschreven kan worden (dus veldleider of instructeur)
    for (const dienst of diensten) {
      ...
      if (!lid?.EMAIL) {
        const html = buildEmailErrorHtml("Dagrapport herinnering, geen e-mail", `<p>${lid.NAAM} heeft een ingeroosterde dienst op ${datum}, maar heeft geen emailadres. Onderneem aktie</p>`);
        await this.googleService.sendHtmlEmail({
          to: process.env.ICT || 'ict@gezc.org',
          subject: 'Dagrapport herinnering, e-mail ontbeekt',
          html
        });
