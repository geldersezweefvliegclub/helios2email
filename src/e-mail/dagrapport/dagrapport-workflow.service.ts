...
export class DagrapportWorkflowService
{
  ...
  async run(forDate = new Date()): Promise<void> {
    ...
    // Zoek uit om welke dagen het gaat. RESULTAAT is door de AuditService al geparseerd naar object.
    const dagen = new Set<string>();
    for (const record of auditRecords) {
      if (record.RESULTAAT?.DATUM) {
        dagen.add(record.RESULTAAT.DATUM);
      }
    }
  ...
  private async sendDagMailForDate(dag: string, ontvangers: LidRecord[]): Promise<void> {
    ...
    // Zorg dat e-mail niet meerdere keren verstuurd wordt naar dezelfde ontvanger
    // Optineel kan e-mail ook verstuurd worden naar AlwaysTo
