 ...
 * de commissaris vliegend. Komt overeen met vliegend.e-mail.php.
 */
@Injectable()
export class VliegendWorkflowService {
  ...
  async run(baseDate = new Date()): Promise<void> {
    ...
    for (const record of auditRecords) {
      ...
      // De dagrapport is wel gewijzigd, maar er is geen aanpassing gemaakt aan vliegend materieel
      ...
      // Vliegend materieel omschrijving moet meer dan 5 woorden bevatten
      if (resultaat.VLIEGENDMATERIEEL.trim().split(/\s+/).length <= 5) {
        continue;
      }
